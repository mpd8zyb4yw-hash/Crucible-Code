...
async function main() {
  ...
  // The EXACT screenshot phrasing: "Open YouTube, search for X and play the third video".
  // The trailing "and" connector (left when the selector is peeled) must not leak into the subject.
  const scr = resolveLocalIntent('Open YouTube, search for be.busta and play the third video')
  assert(scr?.intent === 'search_select_media' && scr?.steps[0]?.args?.query === 'be.busta',
    `screenshot phrasing: subject is "be.busta", not "be.busta and" (got "${scr?.steps[0]?.args?.query}")`)
  assert(await planOpenedUrl('Open YouTube, search for be.busta and play the third video') === 'https://www.youtube.com/watch?v=CCCCCCCCCCC',
    'screenshot phrasing opens the THIRD result')
