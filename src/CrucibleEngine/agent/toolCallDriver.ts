...
/**
 * Tool CATEGORIES — the fix for selection.
 *
 * MEASURED 2026-08-03: a flat 47-name enum is too wide for a 1.5B head. Choosing the next step
 * of "read prices.csv, total it, write total.txt" it reached for `read_image` and `read_pdf`,
 * and never found `compute`. Narrowing the flat list by lexical overlap made it WORSE (2/5 ->
 * 1/5) because the words in a goal do not name the tools a goal needs — "total the column"
 * contains no token resembling `compute`.
 *
 * So the choice is made hierarchically instead: pick an INTENT from a handful, then pick a tool
 * from within that intent. Two 5-to-8-way decisions instead of one 47-way decision, both still
 * grammar-constrained. This is the same move as everywhere else in this file — the system
 * imposes the structure, the model only fills in which branch.
 *
 * Every tool not named here lands in `other`, so a newly registered tool is always reachable
 * rather than silently invisible.
 */
export const TOOL_CATEGORIES: Record<string, string[]> = {
  READ: ['read_file', 'list_dir', 'drive_read', 'read_pdf', 'read_image'],
  WRITE: ['write_file', 'edit_file', 'apply_patch', 'move_file'],
  CALCULATE: ['compute'],
  SEARCH: ['web_search', 'search', 'custom_search', 'knowledge_graph_search'],
  DELETE: ['delete_file', 'delete_folder', 'empty_trash'],
  ASK: ['ask_user'],
}

/** Human-readable hint per category — what the head is actually choosing between. */
const CATEGORY_HINT: Record<string, string> = {
  READ: 'look at a file or list a folder',
  WRITE: 'create or change a file',
  CALCULATE: 'work out a number exactly (sums, totals, percentages)',
  SEARCH: 'find something on the web',
  DELETE: 'remove a file or folder',
  ASK: 'ask the user a question you cannot answer yourself',
  OTHER: 'anything else (running commands, apps, integrations)',
}

/** Group the available tools by category, dropping empty groups. */
export function groupTools(tools: ToolDef[]): Record<string, ToolDef[]> {
  const byName = new Map(tools.map(t => [t.name, t]))
  const claimed = new Set<string>()
  const out: Record<string, ToolDef[]> = {}
  for (const [cat, names] of Object.entries(TOOL_CATEGORIES)) {
    const members = names.map(n => byName.get(n)).filter((t): t is ToolDef => !!t)
    if (members.length) { out[cat] = members; members.forEach(t => claimed.add(t.name)) }
  }
 ...
 *
 * MEASURED 2026-08-03: even with a hierarchical choice, the head picked OTHER (27 tools) and
 * landed on `gmail_search` while trying to total a CSV column. Asking a 1.5B model "what kind
 * of action comes next?" is still asking it to plan, and planning is the thing it is worst at.
 *
 * But the goal already says the order. "Read prices.csv, add up the amount column, and write
 * the total into total.txt" is READ, then CALCULATE, then WRITE, in the order the verbs appear.
 * Extracting that is string work the machine can do exactly, so it does — and the model is left
 * with the one question it can answer: WHICH read, WHICH file, WHICH expression.
 *
 * The plan is a HINT, not a cage: it is consumed one category per successful step, and once
 * exhausted (or if no verb matched) selection falls back to asking. A goal whose verbs we
 * cannot read is not a goal we refuse — it is one where the head gets the wider choice.
 */
const CATEGORY_VERBS: Array<{ cat: string; rx: RegExp }> = [
  { cat: 'READ', rx: /\b(read|open|look at|inspect|list|examine|load|check)\b/gi },
  { cat: 'CALCULATE', rx: /\b(add up|sum|total|calculate|compute|work out|count|average|multiply)\b/gi },
  { cat: 'SEARCH', rx: /\b(search|find out|look up|research|google)\b/gi },
  { cat: 'WRITE', rx: /\b(write|create|save|record|output|put|store|rename|edit|update|replace)\b/gi },
  { cat: 'DELETE', rx: /\b(delete|remove|erase|wipe)\b/gi },
]

export function categoryPlan(goal: string): string[] {
  const hits: Array<{ at: number; cat: string }> = []
  for (const { cat, rx } of CATEGORY_VERBS) {
    for (const m of (goal ?? '').matchAll(rx)) hits.push({ at: m.index ?? 0, cat })
  }
  hits.sort((a, b) => a.at - b.at)
  // Collapse immediate repeats: "read A and read B" is still one READ phase.
  const out: string[] = []
...
export function makeToolCallDriveTurn(complete: Complete, goal: string) {
  return async function toolCallDriveTurn(
    ...
    const usableAll = tools.filter(t => !excluded.has(t.name))
    const pool = usableAll.length ? usableAll : tools

    // ── Stage 0: INTENT ───────────────────────────────────────────────────────────
    // A 47-way choice is where selection broke. Pick a category first — six or seven
    // options with plain-English hints — then choose only within it.
    const groups = groupTools(pool)
    const cats = Object.keys(groups)
    let scopedTools = pool

    // The goal's own verbs decide the order. Steps already completed consume plan entries, so
    // turn 1 of "read … total … write" is READ, turn 2 is CALCULATE, turn 3 is WRITE.
    const plan = categoryPlan(goal)
    const stepsDone = succeeded.size
    const planned = plan[stepsDone]
    if (planned && groups[planned]?.length && attempt === 0) {
      scopedTools = groups[planned]
    } else if (cats.length > 1) {
      const catSystem = [
        'You are executing a task step by step. Choose what KIND of action comes next.',
        '',
        ...cats.map(c => `- ${c}: ${CATEGORY_HINT[c] ?? ''}`),
        `- ${FINISH}: the goal is already fully achieved by the work shown below`,
        '',
        `Reply with EXACTLY ONE of: ${[...cats, FINISH].join(', ')}`,
      ].join('\n')
      const catUser = [
        `GOAL: ${goal}`, '',
        'STEPS ALREADY DONE:', done, '',
        'What kind of action comes next?',
      ].join('\n')
      const cat = (await complete(
        [{ role: 'system', content: catSystem }, { role: 'user', content: catUser }],
        { gbnf: enumGrammar([...cats, FINISH]), maxTokens: 8, temperature: 0, signal },
      )).trim()
      if (cat === FINISH) return { text: 'Goal complete.', toolCalls: [] }
      if (groups[cat]?.length) scopedTools = groups[cat]
    }

    const usable = scopedTools
    const choices = [...(usable.length ? usable : pool).map(t => t.name), FINISH]
    const selectSystem = [
      'You are the executor of a task. You act by choosing ONE tool to call next.',
      '',
      'TOOLS:',
      toolMenu(usable.length ? usable : pool),
      '',
