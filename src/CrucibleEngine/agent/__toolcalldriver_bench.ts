...
const checkDeferred: Array<() => void> = []

async function main() {
  // Happy path: select then fill.
  seen.length = 0
  let drive = makeToolCallDriveTurn(stub(['WRITE', 'write_file', '{"path": "/tmp/a/notes.md", "content": "Crucible agent test"}']), 'Create notes.md')
  ...
  check('stage 0 passes an enum grammar over the CATEGORY names',
    !!seen[0].gbnf && seen[0].gbnf.includes('"WRITE"'),
    seen[0].gbnf ?? 'no grammar')
checkDeferred.push(() => check('stage 1 passes an enum grammar over the tool names',
    !!seen[1].gbnf && seen[1].gbnf!.includes('"write_file"') && seen[1].gbnf!.includes(`"${FINISH}"`),
    seen[1].gbnf ?? 'no grammar'))
  checkDeferred.push(() => check('stage 1 grammar cannot emit prose',
    !!seen[1].gbnf && !/[a-z] [a-z]/.test(seen[1].gbnf.replace(/root ::= /, '')),
    seen[1].gbnf ?? ''))
  checkDeferred.push(() => check('stage 2 passes a JSON-object grammar for the chosen tool only',
    !!seen[2].gbnf && seen[2].gbnf.includes('path') && seen[2].gbnf.includes('content') && !seen[2].gbnf.includes('append'),
    seen[2].gbnf ?? 'no grammar'))
  checkDeferred.forEach(f => f())
  ...
  drive = makeToolCallDriveTurn(stub(['WRITE', 'write_file', 'I cannot do that, sorry.']), 'Create notes.md')
  r = await drive([], TOOLS)
  ...
  drive = makeToolCallDriveTurn(stub(['WRITE', 'write_file', '{"path": "/tmp/a/notes.md"}']), 'Create notes.md')
  r = await drive([], TOOLS)
  check('a partial argument object produces NO call — write_file without content is a different action',
    r.toolCalls.length === 0, JSON.stringify(r.toolCalls))

  drive = makeToolCallDriveTurn(stub(['WRITE', 'I am unable to help with that']), 'Create notes.md')
  r = await drive([], TOOLS)
