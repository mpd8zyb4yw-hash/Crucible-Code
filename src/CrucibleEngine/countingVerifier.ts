...
// Bare "how many r's in strawberry" / "count the a's in banana" — NO verb (are/is/appear), which
// the patterns above all require, so the canonical phrasing fell through. Safe because the needle
// must be an unambiguous SINGLE letter (optionally 's): "how many people in France" (word needle)
// and "how many calories are in a banana" can't match, so this never hijacks a quantity question.
const BARE_LETTER_IN_RX =
  /(?:how\s+many|count)\s+(?:the\s+)?(?:letters?\s+)?["']?([a-zA-Z])(?:'s|s)?["']?\s+in\s+(?:the\s+(?:word|string|name)\s+)?["']?([a-z][a-z0-9\-']*)["']?\s*\??$/i
...
export function answerCountingQuery(message: string): CountingAnswer | null {
  ...
  const m =
    ...
    trimmed.match(BARE_LETTER_IN_RX)
  if (!m) return null
