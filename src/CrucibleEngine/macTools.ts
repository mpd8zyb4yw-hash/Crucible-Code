...
export async function getUITree(): Promise<string> {
  try {
    ...
    return stdout.trim().slice(0, 3000) // cap at 3000 chars for model injection
  } catch (e: any) {
    return `Error reading UI tree: ${e.message?.slice(0, 100)}`
  }
}
...
tell application "System Events"
  ...
  try
    set matchedEl to first UI element of front window of targetApp whose title contains "${targetTitle.replace(/"/g, '\\"')}"
    click matchedEl
    return "Clicked: " & title of matchedEl
  end try
  try
    set matchedEl to first button of front window of targetApp whose title contains "${targetTitle.replace(/"/g, '\\"')}"
    click matchedEl
    return "Clicked button: " & title of matchedEl
  end try
  return "Element not found: ${targetTitle.replace(/"/g, '\\"')}"
end tell`
