...
/**
 * Molten pour animation — ported nearly verbatim from the Crucible v3.dc.html
 * prototype's `drawPour`/`drawHalf`/`roundRectPoints`/`moltenColor`/`drawVessel`.
 *
 * Three phases, hooked to the REAL stream lifecycle (never a fixed timer):
 *   thinking → pouring (first token) → finishing (stream done) → cooling
 * `finishing`/`cooling` transitions and the terminal `finalizeLive()` call are
 * driven by the animator itself, exactly as in the prototype, because the
 * fill/cool progress (not the raw network event) is what has to finish before
 * the message is allowed to "land".
 */
export default function MoltenPour({ wrapRef }: { wrapRef: React.RefObject<HTMLDivElement> }) {
  const canvasRef = useRef<HTMLCanvasElement>(null)

  // animator internals — refs, not React state, so nothing here triggers re-renders
  const fill = useRef(0)
  ...
  const sawPouring = useRef(false)
  const sawCooling = useRef(false)
  ...
  useEffect(() => {
    ...
    const moltenColor = (s: number, t: number, alpha: number) => {
      ...
      const r = 255
      const g = Math.round(70 + n * 110)
      const b = Math.round(10 + n * 60)
    ...
    const drawVessel = (ctx: CanvasRenderingContext2D, alpha: number) => {
      ...
      ctx.beginPath()
      ctx.moveTo(-6.5, 9.5)
      ctx.quadraticCurveTo(0, 14, 6.5, 9.5)
      ctx.stroke()
      ctx.beginPath()
      ctx.moveTo(-11, -8)
      ctx.lineTo(11, -8)
    ...
    const frame = (now: number) => {
      ...
      const live = useCrucibleStore.getState().live
      if (canvas && wrap && live) {
        ...
        const rad = 15
        const { minFillMs } = useCrucibleStore.getState()

        if (ph === 'pouring' && !sawPouring.current) {
          sawPouring.current = true
          pourStart.current = now
        }
        if (ph === 'cooling' && !sawCooling.current) {
          sawCooling.current = true
          coolStart.current = now
        }
        ...
        // ── fill progress: real chars-received fraction, smoothed, min-duration floor ──
        if (ph === 'pouring' || ph === 'finishing') {
          const elapsed = now - pourStart.current
          const cap = elapsed / Math.max(400, minFillMs)
          const raw = ph === 'finishing' ? 1 : live.totalChars > 0 ? live.text.length / live.totalChars : 0
          fillTarget.current = Math.min(1, raw, cap)
          const delta = fillTarget.current - fill.current
          ...
          if (ph === 'finishing' && fill.current > 0.995) {
            fill.current = 1
            useCrucibleStore.getState().setLivePhase('cooling')
          }
        }

        // ── cooling sweep + crucible fade, concurrent, min-duration floor ──
        if (ph === 'cooling') {
          const coolDur = Math.max(1000, 800)
          cool.current = Math.min(1, (now - coolStart.current) / coolDur)
          cruOp.current = Math.max(0, 1 - cool.current * 1.05)
          if (cool.current >= 1) {
            useCrucibleStore.getState().finalizeLive()
