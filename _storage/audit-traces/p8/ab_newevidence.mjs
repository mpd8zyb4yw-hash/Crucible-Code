...
const EVIDENCE = readFileSync(join(HERE, '..', 'p8', 'newevidence.txt'), 'utf8')
...
const armList = (process.env.ARMS ?? 'fm,bonsai').split(',')
...
for (const arm of armList) {
  for (let i = 0; i < N; i++) {
    const tag = `${arm}-${i}`
    process.stderr.write(`[${tag}] asking…\n`)
    let r
    try { r = await ask(arm, 1000 + i) } catch (e) {
      process.stderr.write(`[${tag}] ERROR ${e.message}\n`)
      report.push({ arm, run: i, error: String(e.message) }); continue
    }
    writeFileSync(join(OUT, `${tag}.answer.md`), r.text)
    if (r.reasoning) writeFileSync(join(OUT, `${tag}.reasoning.md`), r.reasoning)
    if (r.truncated) {
      process.stderr.write(`[${tag}] ${(r.ms/1000).toFixed(1)}s  TRUNCATED (finish=length, no answer) — EXCLUDED\n`)
      report.push({ arm, run: i, ms: r.ms, truncated: true, finish: r.finish, usage: r.usage })
      continue
    }
    const ex = extraction(r.text)
    const ev = execute(r.text, tag)
    const row = { arm, run: i, ms: r.ms, chars: r.text.length, finish: r.finish, usage: r.usage, ...ex, ...ev }
    report.push(row)
    process.stderr.write(
      `[${tag}] ${(r.ms/1000).toFixed(1)}s  z.ipv4=${ex.usesZIpv4}  executes=${ev.executesCorrectly}  ` +
      `jsonSchema=${ex.jsonSchema} regex=${ex.handRolledRegex}\n`
    )
  }
}

writeFileSync(join(OUT, 'ab_report.json'), JSON.stringify(report, null, 2))
console.log('\n=== cont.88 A/B: copy-from-evidence ===')
for (const arm of armList) {
  const all = report.filter(r => r.arm === arm)
  const trunc = all.filter(r => r.truncated).length
  const errs = all.filter(r => r.error).length
  const rows = all.filter(r => !r.error && !r.truncated)
  if (trunc || errs) console.log(`${arm.padEnd(14)} NOTE: ${trunc} truncated, ${errs} errored — excluded from rates below`)
  if (!rows.length) { console.log(`${arm.padEnd(14)} NO SCORABLE RUNS (nothing measured — not a model failure)`); continue }
  const n = rows.length
  const pct = k => `${k}/${n}`
  console.log(
    `${arm.padEnd(14)} n=${n}  z.ipv4()=${pct(rows.filter(r => r.usesZIpv4).length)}  ` +
    `executes=${pct(rows.filter(r => r.executesCorrectly).length)}  ` +
    `jsonSchema=${pct(rows.filter(r => r.jsonSchema).length)}  ` +
    `handRegex=${pct(rows.filter(r => r.handRolledRegex).length)}  ` +
    `avg=${(rows.reduce((s, r) => s + r.ms, 0) / n / 1000).toFixed(1)}s`
  )
}
