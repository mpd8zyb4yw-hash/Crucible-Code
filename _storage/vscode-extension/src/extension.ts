...
// Three editor-context commands send the current selection to the user's Crucible
// pipeline (POST ${endpoint}/api/chat, body { message, mode: 'auto' }) and render the
// streamed synthesis — plus any critic findings and confidence calibration — into a
// dark WebviewPanel styled like the Crucible app. Dependency-light: only the `vscode`
// module and the global `fetch` provided by VS Code's Node 18+ runtime.

type CommandKind = 'review' | 'explain' | 'improve';

interface CommandSpec {
  id: string;
  kind: CommandKind;
  title: string;
  // Builds the message sent to the pipeline from the selected code + its language id.
  buildPrompt(code: string, languageId: string): string;
}
...
const COMMANDS: CommandSpec[] = [
  {
    ...
    buildPrompt: (code, lang) =>
      `Review the following ${lang} code. Identify bugs, edge cases, security issues, and design problems. ` +
  ...
  },
  {
    ...
    buildPrompt: (code, lang) =>
      `Explain what the following ${lang} code does, step by step, including its purpose, inputs, ` +
  ...
  },
  {
    ...
    buildPrompt: (code, lang) =>
      `Improve the following ${lang} code. Provide a cleaner, safer, more idiomatic version and briefly ` +
      `explain each change. Keep behavior equivalent unless a bug requires a fix.\n\n\`\`\`${lang}\n${code}\n\`\`\``,
  },
];

// Result accumulated from the SSE stream.
interface PipelineResult {
  ...
  confidence?: {
    ...
    flaggedClaims?: Array<{ claim: string; tier: string }>;
  };
...
export function activate(context: vscode.ExtensionContext): void {
  for (const spec of COMMANDS) {
    context.subscriptions.push(
      vscode.commands.registerCommand(spec.id, () => runCommand(spec, context)),
    );
  }
}
...
async function runCommand(spec: CommandSpec, context: vscode.ExtensionContext): Promise<void> {
  ...
  const endpoint = (config.get<string>('endpoint') || 'https://crucible.cam').replace(/\/+$/, '');
  const apiKey = config.get<string>('apiKey') || '';

  if (!apiKey) {
    const pick = await vscode.window.showWarningMessage(
      'Crucible: no API key set. Add your session JWT in Settings (crucible.apiKey).',
      'Open Settings',
    );
  ...
  const panel = vscode.window.createWebviewPanel(
    'crucibleResult',
    `Crucible: ${labelForKind(spec.kind)}`,
...
function labelForKind(kind: CommandKind): string {
...
// ── Pipeline streaming ───────────────────────────────────────────────────────
// POSTs to ${endpoint}/api/chat and reads the SSE response. Each event is a
// `data: {json}` line. We collect synthesis tokens, final synthesis text, critic
// findings, and confidence calibration. `onUpdate` is called as state changes so the
// webview can render progressively.

interface StreamArgs {
  endpoint: string;
  apiKey: string;
  message: string;
  onUpdate(result: PipelineResult, done: boolean): void;
}

async function streamPipeline(args: StreamArgs): Promise<void> {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Pipeline returned ${response.status} ${response.statusText}` +
  ...
  const result: PipelineResult = { synthesis: '', criticProblems: [] };
        ...
        switch (event.type) {
          ...
          case 'critic':
            if (Array.isArray(event.problems)) {
              result.criticProblems = event.problems.map((p: unknown) => String(p));
              flush(false);
            }
            break;
          default:
...
// ── Webview rendering ────────────────────────────────────────────────────────
// Static HTML (enableScripts: false). Styled to match the Crucible app's dark theme.
// No emojis anywhere. All dynamic text is HTML-escaped.
...
function renderHtml(args: RenderArgs): string {
  ...
  if (status === 'error') {
    body += `<section class="card error"><h2>Request failed</h2><pre class="synthesis">${escapeHtml(
      result.error || 'Unknown error.',
    )}</pre></section>`;
  } else {
    const synthesis = result.synthesis.trim();
    if (synthesis) {
      body += `<section class="card"><h2>Synthesis</h2><div class="synthesis">${renderText(
        synthesis,
      )}</div></section>`;
...
function renderConfidence(conf: NonNullable<PipelineResult['confidence']>): string {
