...
function resolveSearchAndSelect(m: string): LocalPlan | null {
  ...
  subject = subject
    ...
    .replace(/^\s*for\s+/i, '')
    .replace(/[,;:—-]+\s*$/, '')
    // A trailing connector left when the selector was peeled off the end
    // ("search for X and play the third video" → subject "X and" → "X").
...
// A GENUINELY compound request ("open settings, set brightness, THEN play a video") must not
// be served by a single-action resolver — that silently drops every step after the first, so
// we defer to the full LLM loop. BUT "search youtube for X and play the third video" only LOOKS
// compound: it is one coherent search-then-select action the deterministic layer handles
// correctly (resolveSearchAndSelect), so it is tried BEFORE this precision bail. Removing the
// bail entirely (as the search-select work first did) would regress compound system commands.
...
export async function runLocalPlan(
  ...
  const base = plan.steps.length === 1 && lastOut ? lastOut : plan.label
  const note = corrections.length ? ` (self-corrected: ${corrections.join('; ')})` : ''
  return { ok: true, outputs, corrections, summary: base + note }
}
