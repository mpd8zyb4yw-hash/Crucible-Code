...
## CURRENT STATE (last updated 2026-07-16 cont.81 — Phase 2 of the audit: **is retrieval-as-brain wired? Split verdict.** (1) **CODE path: NOT WIRED.** `CRUCIBLE_CODE_WEB_GROUND` occurs in 2 places, both server.ts, **assigned nowhere** → `withRetrieval()` silently returns the base proposer. Needs `CRUCIBLE_CONVERGE=1` too — three stacked gates. Forced ON it fetches a README (not source) → "no function definitions extracted". On **app builds it is not even on the call graph** (synthDriver never calls solveCodeTask). (2) **ANSWER path: WIRED, TWO BUGS.** **Routing (OPEN, top priority):** grounding fires only for QUESTION-shaped prompts — "Write a Zod schema…" bypasses it and returns **JSON Schema, wrong library**. This root-causes cont.80b/Phase-1's "zero grounding fired", which **STANDS**. Contradicts cont.69's "ground-by-default for EVERY query". **Truncation (FIXED):** `PER_SOURCE_CHARS=1200` head-sliced evidence — correct zod page stripped to 7955 chars with `ipv4` at offset **6370**, so the model got 3.6KB of zod evidence with **zero** ipv4 mentions and grafted a `zipCode` regex it DID find; it obeyed its prompt, the pipeline lied to it. Fixed with `selectRelevantPassages()` (IDF-weighted 400-char windows, head-slice fallback when no terms match). **Live-verified `regex(/^\d{5}$/)` → `ipv4()` through real /api/chat.** ground:bench 24/24, vgr:bench 208/208. **THE FRAME: retrieval FINDS correctly and keylessly (no paid API, nothing missing from .env.local) — the answer is lost between fetch and model. Extraction is the frontier, not search.** Next: **Phase 2a — route code-shaped prompts through grounding** (find the shape gate; cont.53/69 point at isResearchShaped/isCodeShaped in solveNonCodeTurn). See CRUCIBLE_AUDIT_PHASE2.md §7 for the phased plan.)
