...
**TOP NEXT ITEMS:** (1) **per-turn verification/council latency** — the remaining latency cost on PARAMETRIC (non-grounded) turns: factConsensus/explainCheck lanes + council debate are extra serialized FM calls. Consider backgrounding the council (it's display-only) or streaming/parallelizing the verification lanes. Speed-vs-council-visibility is a user tradeoff. (2) source-RANKING quality (grounded useEffect answer muddled by mediocre SO-thread pick; "sky blue" chose the color article). (3) wire a real agentic consumer for MiniCPM. (4) re-verify absolute latencies on a SINGLE server (this session's numbers were inflated by two instances sharing one FM daemon).
