...
**TOP NEXT ITEMS:** (1) **per-turn verification/council latency** — the remaining latency cost: factConsensus/explainCheck lanes + council debate are multiple extra serialized FM calls (~25s for a simple direct lookup under 2-server contention). Consider: background the council (don't block the turn), or make the verification lanes stream/parallel, or tighten their gates. Speed-vs-council-visibility is a user tradeoff. (2) source-RANKING quality (grounded useEffect answer muddled by mediocre SO-thread pick; "sky blue" chose the color article). (3) wire a real agentic consumer for MiniCPM. (4) also stream the DIRECT (non-grounded) answer path for consistency.
